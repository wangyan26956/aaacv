...
function showInChat(text: string): void {
  ensureChatOpen();
  if (postToWebview) {
    postToWebview({ t: 'm', r: 'assistant', x: text });
  }
}
...
export function registerCommands(context: vscode.ExtensionContext): void {
  ...
  subs.push(
    vscode.commands.registerCommand('claudeCode.generateCode', async () => {
      const desc = await vscode.window.showInputBox({
        ...
        placeHolder:
          'e.g., A Python function that sorts a list of dictionaries by a key',
      });
      if (!desc) return;
      ensureChatOpen();
      const ctx = await getFullContext();
  ...
  subs.push(
    vscode.commands.registerCommand('claudeCode.newChat', () => {
      newChat();
      if (postToWebview) postToWebview({ t: 'clear' });
  ...
  subs.push(
    vscode.commands.registerCommand('claudeCode.toggleChat', () => {
      vscode.commands.executeCommand(
        'workbench.view.extension.claude-code-sidebar',
      );
    }),
  );
}
